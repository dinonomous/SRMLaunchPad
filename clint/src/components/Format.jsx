...
function Format({ children, Notebool, toggleNotebool, breakpoint}) {
  ...
  function fetchData () {
    fetch("http://192.168.30.213/getcollectionnames")
      .then((response) => response.json())
      .then((data) => {
        const { collectionNames } = data;
        console.log("Data received from backend:", data);
  ...
  function fetchDataQuiz () {
    fetch("http://192.168.30.213/api/quizapi/getcollectionnames")
      .then((response) => response.json())
      .then((data) => {
        const { collectionNames } = data;
        console.log("Data received from backend:", data);
  ...
  // useEffect(()=>{setCollectionData([])},[Subjects])
  // useEffect(()=>{setCollectionData([])},[QuizSubjects])
  ...
  return (
    ...
    //           fill="#F8F4E3"
    //         >
    //           <title xmlns="http://www.w3.org/2000/svg">collections_07</title>
    //           <path
    ...
    //           </li>
    //           <li onMouseEnter={handleMouseEnterQuiz}>
    //             <Link to="#">Quiz</Link>
    //           </li>
    //           <li>
    //             <Link to="#" onClick={toggleNotebool}>
    //               Notes
    //             </Link>
    //           </li>
    ...
    //     </div>
    //     <div
    //       className={`subjects row ${isVisible ? "" : "hidden"}`}
    //       ref={bellowRef}
    ...
    //                 onClick={() =>
    //                   setcurrentLink(
    //                     item.startsWith("quiz") ? "" : `/${item}/Unit 1`
    //                   )
    //                 }
    //               >
    //                 {item}
    ...
    //   </nav>
    //   <main Notebool={Notebool} ref={childRef} className="main_body">
    //     {children}
    //   </main>
    //   <footer></footer>
    // </>
