...
const Login = () => {
  ...
  const handleCheckEmail = async () => {
    if (!email.includes("@")) {
      ...
      try {
        const response = await fetch(
          "http://localhost:5001/api/v2/authentication/check-email",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        <div className="max-w-5xl h-full w-full relative flex items-center">
          ...
          <div className="m-auto min-h-fit h-1/2 w-9/12">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full text-DarkPrimary-100 h-[3.5rem] dark:bg-emerald-500 rounded-xl font-semibold hover:bg-slate-600 transition-colors"
              >
            ...
            <div className="h-[13rem] flex flex-col mt-4 w-3xl">
              ...
              <div className="mt-5 text-lg border-b border-neutral-300 py-4 text-neutral-300">
                <a href="#">Login with SRM mail id for future suprise</a>
              </div>
              <div className="mt-3 text-lg flex justify-between items-center text-neutral-300">
                <p>Alredy have an account?</p>
                <button
                  type="button"
                  className="text-lg h-12  dark:bg-[#235789] rounded-full text-white bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium px-8 py-2.5 me-2 mb-2 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                >
                  Login
                </button>
              </div>
